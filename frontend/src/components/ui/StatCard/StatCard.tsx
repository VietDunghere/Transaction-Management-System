...
export function StatCard({ label, value, change, changeType = 'neutral', accent = 'purple' }: StatCardProps) {
    return (
        <div
            ...
            style={{ padding: 24 }}
        >
            <span className="text-sm font-normal text-text-on-accent truncate">{label}</span>
            <span className="text-2xl font-semibold leading-8 text-text-on-accent">{value}</span>
            {change && (
                <span
                    className={cn(
                        'text-sm',
