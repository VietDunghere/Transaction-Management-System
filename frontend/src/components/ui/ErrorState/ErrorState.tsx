...
export function ErrorState({ type = 'unknown', title, description, onRetry, className }: ErrorStateProps) {
    ...
    return (
        ...
        >
            <div className="mb-5 flex size-16 items-center justify-center rounded-full bg-feedback-danger-bg text-status-danger">
                {config.icon}
            </div>
            <h3 className="text-base font-semibold mb-2 text-text-primary">{title || config.title}</h3>
            <p className="text-sm text-text-secondary max-w-sm leading-relaxed">{description || config.description}</p>
            {onRetry && (
                <div className="mt-6">
