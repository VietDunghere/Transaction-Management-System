...
--   • Gộp roles + user_roles → cột role trong tbl_users (mỗi người 1 vai trò)
--   • Gộp risk_scoring_results → fraud_score + model_version trong tbl_transactions_live
--   • Loại merch_lat/long khỏi tbl_transactions_live (3NF: phụ thuộc bắc cầu qua merchant_id)
--   • Loại unix_time (đạo hàm từ txn_time), source_ip, reason_code, override_reason
--   • Đơn giản tbl_review_cases.case_status: OPEN | ASSIGNED | CLOSED (tách decision riêng)
...
--     dim_location, fact_transactions, fact_tbl_loans
...
--   • tbl_loans.person_* là snapshot tại thời điểm nộp → KHÔNG phải phụ thuộc
...
--    Đã loại: merch_lat/long (3NF: phụ thuộc bắc cầu → tbl_merchants),
--             unix_time (đạo hàm), source_ip, reason_code, override_reason
CREATE TABLE tbl_transactions_live (
...
CREATE TABLE tbl_model_configs (
  ...
  version     number        DEFAULT 1 NOT NULL,
  CONSTRAINT uq_tbl_model_configs_name_param UNIQUE (model_name, param_name)
);
...
-- tbl_users
CREATE INDEX idx_tbl_users_role            ON tbl_users (role);
CREATE INDEX idx_tbl_users_status          ON tbl_users (status);

-- tbl_transactions_live
CREATE INDEX idx_txn_live_status       ON tbl_transactions_live (status);
CREATE INDEX idx_txn_live_time         ON tbl_transactions_live (txn_time);
CREATE INDEX idx_txn_live_cust         ON tbl_transactions_live (customer_id);
CREATE INDEX idx_txn_live_merch        ON tbl_transactions_live (merchant_id);
CREATE INDEX idx_txn_live_chan         ON tbl_transactions_live (channel_id);
CREATE INDEX idx_txn_live_card_hash    ON tbl_transactions_live (card_number_hash);
CREATE INDEX idx_txn_live_submitted    ON tbl_transactions_live (submitted_by);

-- tbl_review_cases
CREATE INDEX idx_case_status           ON tbl_review_cases (case_status);
CREATE INDEX idx_case_assigned         ON tbl_review_cases (assigned_to);

-- tbl_loans
CREATE INDEX idx_tbl_loans_status          ON tbl_loans (status);
CREATE INDEX idx_tbl_loans_customer        ON tbl_loans (customer_id);
CREATE INDEX idx_tbl_loans_submitted       ON tbl_loans (submitted_by);
CREATE INDEX idx_tbl_loans_reviewed_by     ON tbl_loans (reviewed_by);

-- tbl_audit_logs
CREATE INDEX idx_audit_event_type      ON tbl_audit_logs (event_type);
CREATE INDEX idx_audit_entity          ON tbl_audit_logs (entity_type, entity_id);
CREATE INDEX idx_audit_event_ts        ON tbl_audit_logs (event_ts);
CREATE INDEX idx_audit_actor           ON tbl_audit_logs (actor_user_id);
...
COMMENT ON TABLE  tbl_users IS 'Tài khoản nhân viên — role gộp từ bảng roles (mỗi người 1 vai trò theo UC06.3)';
COMMENT ON COLUMN tbl_users.user_id IS 'UUID';
COMMENT ON COLUMN tbl_users.role    IS 'OPERATOR | REVIEWER | ANALYST | MANAGER | ADMIN';
COMMENT ON COLUMN tbl_users.status  IS 'ACTIVE | DISABLED';

COMMENT ON TABLE  tbl_customers IS 'Dữ liệu tham chiếu — nạp sẵn từ core banking, không có UC quản lý';
COMMENT ON COLUMN tbl_customers.customer_id IS 'UUID';

COMMENT ON TABLE  tbl_merchants IS 'Dữ liệu tham chiếu — nạp sẵn, không có UC quản lý';
COMMENT ON COLUMN tbl_merchants.merchant_id IS 'UUID';

COMMENT ON TABLE  tbl_channels IS 'Dữ liệu tham chiếu — nạp sẵn (POS, ATM, Online, Mobile…)';

COMMENT ON TABLE  tbl_transactions_live IS 'Giao dịch tài chính — fraud_score + model_version gộp từ risk_scoring_results';
COMMENT ON COLUMN tbl_transactions_live.txn_id           IS 'UUID';
COMMENT ON COLUMN tbl_transactions_live.status           IS 'PENDING | APPROVED | REJECTED | MANUAL_REVIEW';
COMMENT ON COLUMN tbl_transactions_live.card_number_hash IS 'SHA256 hash — không lưu số thẻ raw';
COMMENT ON COLUMN tbl_transactions_live.fraud_score      IS 'Điểm gian lận từ AI model (0.0–1.0)';
COMMENT ON COLUMN tbl_transactions_live.model_version    IS 'Phiên bản model AI tại thời điểm scoring';

COMMENT ON TABLE  tbl_review_cases IS 'Hồ sơ xét duyệt thủ công — tạo tự động khi txn.status = MANUAL_REVIEW';
COMMENT ON COLUMN tbl_review_cases.case_id     IS 'UUID';
COMMENT ON COLUMN tbl_review_cases.case_status IS 'OPEN | ASSIGNED | CLOSED';
COMMENT ON COLUMN tbl_review_cases.decision    IS 'NULL (chưa quyết) | APPROVE | REJECT';
COMMENT ON COLUMN tbl_review_cases.version     IS 'Optimistic Locking — tăng mỗi lần cập nhật';

COMMENT ON TABLE  tbl_loans IS 'Hồ sơ vay — person_* là snapshot tại thời điểm nộp, không phải phụ thuộc bắc cầu';
COMMENT ON COLUMN tbl_loans.loan_id       IS 'UUID';
COMMENT ON COLUMN tbl_loans.status        IS 'PENDING | APPROVED | REJECTED | DISBURSED | CLOSED | DEFAULTED';
COMMENT ON COLUMN tbl_loans.version       IS 'Optimistic Locking';
COMMENT ON COLUMN tbl_loans.pd_score      IS 'Probability of Default từ AI model (0.0–1.0)';
COMMENT ON COLUMN tbl_loans.model_version IS 'Phiên bản model AI tại thời điểm scoring';

COMMENT ON TABLE  tbl_model_configs IS 'Ngưỡng phân loại Fraud / PD Score — ANALYST điều chỉnh (UC07)';
COMMENT ON COLUMN tbl_model_configs.model_name IS '"fraud" | "loan"';
COMMENT ON COLUMN tbl_model_configs.param_name IS '"reject_threshold" | "review_threshold" | "high_risk_threshold" | "medium_risk_threshold"';

COMMENT ON TABLE  tbl_audit_logs IS 'Nhật ký kiểm toán toàn hệ thống (UC05.2) — thay thế txn_state_history + review_case_actions';
COMMENT ON COLUMN tbl_audit_logs.log_id IS 'UUID';
...
-- tbl_transactions_live → tbl_customers, tbl_merchants, tbl_channels, tbl_users
ALTER TABLE tbl_transactions_live ADD CONSTRAINT fk_txn_customer
  FOREIGN KEY (customer_id)  REFERENCES tbl_customers (customer_id);
ALTER TABLE tbl_transactions_live ADD CONSTRAINT fk_txn_merchant
  FOREIGN KEY (merchant_id)  REFERENCES tbl_merchants (merchant_id);
ALTER TABLE tbl_transactions_live ADD CONSTRAINT fk_txn_channel
  FOREIGN KEY (channel_id)   REFERENCES tbl_channels  (channel_id);
ALTER TABLE tbl_transactions_live ADD CONSTRAINT fk_txn_submitted_by
  FOREIGN KEY (submitted_by) REFERENCES tbl_users     (user_id);

-- tbl_review_cases → tbl_transactions_live, tbl_users
ALTER TABLE tbl_review_cases ADD CONSTRAINT fk_case_txn
  FOREIGN KEY (txn_id)      REFERENCES tbl_transactions_live (txn_id);
ALTER TABLE tbl_review_cases ADD CONSTRAINT fk_case_assigned
  FOREIGN KEY (assigned_to) REFERENCES tbl_users             (user_id);

-- rule_hits → tbl_transactions_live
ALTER TABLE rule_hits ADD CONSTRAINT fk_rule_hit_txn
  FOREIGN KEY (txn_id) REFERENCES tbl_transactions_live (txn_id);

-- tbl_loans → tbl_customers, tbl_users (submitted_by), tbl_users (reviewed_by)
ALTER TABLE tbl_loans ADD CONSTRAINT fk_loan_customer
  FOREIGN KEY (customer_id)  REFERENCES tbl_customers (customer_id);
ALTER TABLE tbl_loans ADD CONSTRAINT fk_loan_submitted_by
  FOREIGN KEY (submitted_by) REFERENCES tbl_users     (user_id);
ALTER TABLE tbl_loans ADD CONSTRAINT fk_loan_reviewed_by
  FOREIGN KEY (reviewed_by)  REFERENCES tbl_users     (user_id);

-- tbl_model_configs → tbl_users
ALTER TABLE tbl_model_configs ADD CONSTRAINT fk_config_updated_by
  FOREIGN KEY (updated_by) REFERENCES tbl_users (user_id);

-- tbl_audit_logs → tbl_users
ALTER TABLE tbl_audit_logs ADD CONSTRAINT fk_audit_actor
  FOREIGN KEY (actor_user_id) REFERENCES tbl_users (user_id);
...
BEGIN
    -- B1: Lấy ngưỡng hiện hành từ tbl_model_configs
    BEGIN
        SELECT param_value INTO v_reject_threshold
        FROM tbl_model_configs
    ...
    INSERT INTO tbl_transactions_live (
...
BEFORE INSERT ON tbl_transactions_live
FOR EACH ROW
BEGIN
...
AFTER UPDATE ON tbl_transactions_live
FOR EACH ROW
BEGIN
