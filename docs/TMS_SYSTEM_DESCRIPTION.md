# Mô tả Hệ thống Phân tích Rủi ro và Đánh giá Tài chính bằng Ngôn ngữ Tự nhiên
...
***Bước 1: Mục đích của hệ thống***: Hệ thống phần mềm **Hệ thống Phân tích Rủi ro và Đánh giá Tài chính (HPTRRĐGTC)** phục vụ công tác quản lý toàn bộ vòng đời giao dịch ngân hàng — từ khi một giao dịch được nộp vào đến khi có quyết định duyệt hoặc từ chối — bao gồm: tự động chấm điểm rủi ro gian lận (Fraud Scoring) và rủi ro tín dụng (Credit Risk Scoring) bằng mô hình AI, phân luồng giao dịch sang xử lý tự động hoặc chuyển cho nhân viên xem xét thủ công, quản lý vòng đời hồ sơ tín dụng (Loan Application), lưu vết toàn bộ hành động nghiệp vụ (Audit Logging), đối soát dữ liệu giữa các lớp hệ thống (Reconciliation), vận hành pipeline nạp dữ liệu từ Data Lake vào Kho dữ liệu thống kê (ETL), và cung cấp Dashboard & Báo cáo phục vụ quản trị. Ngoài ra, hệ thống còn cung cấp một **Analyst Module** chuyên biệt dành cho chuyên viên phân tích rủi ro, cho phép theo dõi hiệu suất mô hình, điều chỉnh ngưỡng phân loại, quản lý suppression rules, và soạn thảo báo cáo phân tích dạng Markdown để gửi lên cấp quản lý dưới dạng PDF chuyên nghiệp.
...
* **OPERATOR** (Core Banking System của Ngân hàng đối tác):
  * Đây **không phải** nhân viên nội bộ — OPERATOR là hệ thống core banking của ngân hàng đối tác duy nhất, gọi API của HPTRRĐGTC theo thời gian thực
  * Nộp giao dịch mới vào hệ thống để chấm điểm fraud và phân luồng tự động
  * Xem danh sách và chi tiết toàn bộ giao dịch trong hệ thống
  * Nộp hồ sơ đề nghị vay vốn (Loan Application) để lấy PD Score
  * Xem danh sách và chi tiết toàn bộ hồ sơ vay trong hệ thống

* **REVIEWER** (Nhân viên duyệt hồ sơ):
  * Được thực hiện các chức năng như thành viên hệ thống
  * Xem danh sách giao dịch và chi tiết từng giao dịch (chỉ đọc)
  * Xem danh sách hồ sơ vay và chi tiết từng hồ sơ (chỉ đọc)
...
* **ANALYST** (Chuyên viên phân tích rủi ro — nhân viên nội bộ HPTRRĐGTC):
  * Được thực hiện các chức năng như thành viên hệ thống
  * Xem danh sách và chi tiết toàn bộ giao dịch, hồ sơ vay, case (chỉ đọc)
  * Xem Audit Log hệ thống (chỉ đọc)
  * Xem Dashboard tổng quan và biểu đồ xu hướng fraud
  * Xuất báo cáo giao dịch và báo cáo fraud (CSV/JSON)
  * Xem danh sách và chi tiết phiên đối soát (Reconciliation) — chỉ đọc
  * Xem ngưỡng phân loại hiện tại của fraud model và credit model
  * Điều chỉnh ngưỡng phân loại (reject_threshold, review_threshold, high_risk_threshold, medium_risk_threshold)
  * Xem thống kê hiệu suất fraud model: phân phối điểm, tỷ lệ false positive theo khoảng thời gian
  * Xem thống kê hiệu suất credit model: phân phối rủi ro, tỷ lệ phê duyệt/từ chối
  * Tạo và quản lý suppression rules (whitelist bypass fraud scoring cho merchant/customer/card VIP)
  * Vô hiệu hóa suppression rule khi không còn cần thiết
  * Soạn báo cáo phân tích dạng Markdown và submit lên MANAGER để xem xét
  * Xem danh sách và chi tiết báo cáo của mình
  * Tải báo cáo dưới dạng PDF chuyên nghiệp
...
* **MANAGER** (Quản lý):
  ...
  * Xem xét và xác nhận (acknowledge) báo cáo phân tích từ ANALYST, kèm ghi chú phản hồi
...
* *Xem danh sách giao dịch*: OPERATOR đăng nhập → chọn chức năng xem giao dịch → hệ thống hiển thị danh sách toàn bộ giao dịch trong hệ thống, được phân trang, gồm thông tin: mã giao dịch, thời gian, số tiền, tên merchant, kênh, điểm gian lận, trạng thái hiện tại → OPERATOR có thể lọc theo khoảng thời gian, trạng thái, kênh, merchant → OPERATOR click vào một giao dịch để xem chi tiết đầy đủ (bao gồm lý do từ chối nếu có, mã case liên quan nếu có).
...
* *Phê duyệt / Từ chối hồ sơ vay*: REVIEWER xem danh sách hồ sơ vay đang PENDING → chọn hồ sơ cần xét duyệt → đọc thông tin khách hàng, PD Score và risk_level → gọi endpoint `PATCH /loans/{loan_id}/decision` kèm: `decision` (APPROVE/REJECT), `review_note` (bắt buộc), `version` (Optimistic Locking) → hệ thống kiểm tra **ràng buộc SoD**: nếu người phê duyệt trùng với người tạo đơn → 403 Forbidden → nếu hợp lệ: khi APPROVE hệ thống tính `monthly_payment`, `outstanding_balance`, `maturity_date`; khi REJECT ghi lý do → hệ thống ghi Audit Log (LOAN_APPROVED / LOAN_REJECTED).
...
* *Xem ngưỡng phân loại*: ANALYST đăng nhập → chọn chức năng Analyst → xem ngưỡng hiện tại của fraud model (reject_threshold, review_threshold) và credit model (high_risk_threshold, medium_risk_threshold) cùng thông tin người cập nhật lần cuối.

* *Điều chỉnh ngưỡng phân loại*: ANALYST chọn chức năng cập nhật ngưỡng → nhập giá trị mới cho một hoặc nhiều tham số → hệ thống thực hiện cross-validation trước khi lưu: review_threshold phải nhỏ hơn reject_threshold; medium_risk_threshold phải nhỏ hơn high_risk_threshold → nếu vi phạm: trả về lỗi 422 với thông báo cụ thể → nếu hợp lệ: cập nhật cấu hình và ghi AuditLog(THRESHOLD_UPDATED) kèm actor và timestamp → ANALYST nhận phản hồi với giá trị mới.

* *Xem thống kê hiệu suất fraud model*: ANALYST chọn chức năng Model Performance → Fraud → chọn khoảng thời gian (1–365 ngày) → hệ thống tổng hợp: số giao dịch APPROVED / MANUAL_REVIEW / REJECTED, tỷ lệ tương ứng, số lượng và tỷ lệ false positive (case MANUAL_REVIEW mà reviewer quyết định APPROVE) → ANALYST dùng thông tin này để đánh giá hiệu quả mô hình và quyết định có cần điều chỉnh ngưỡng hay không.

* *Xem thống kê hiệu suất credit model*: Tương tự fraud model — ANALYST xem phân phối rủi ro (LOW/MEDIUM/HIGH), tỷ lệ phê duyệt/từ chối/pending của các khoản vay, cùng ngưỡng hiện tại.

* *Tạo suppression rule*: ANALYST xác định một merchant/customer/card cần được bypass fraud scoring (ví dụ: merchant VIP có lịch sử giao dịch sạch 5 năm) → chọn loại rule (MERCHANT / CUSTOMER / CARD), nhập entity_id và lý do (bắt buộc), tùy chọn ngày hết hạn → hệ thống lưu rule với is_active = TRUE và ghi AuditLog(SUPPRESSION_RULE_CREATED) → mọi giao dịch từ entity này sẽ tự động bypass fraud scoring cho đến khi rule hết hạn hoặc bị vô hiệu hóa.

* *Vô hiệu hóa suppression rule*: ANALYST xem danh sách suppression rules đang hoạt động → chọn rule cần vô hiệu hóa → xác nhận → hệ thống đặt is_active = FALSE và ghi AuditLog(SUPPRESSION_RULE_DEACTIVATED).

* *Soạn và submit báo cáo phân tích*: ANALYST soạn báo cáo dạng Markdown (hỗ trợ bảng, heading, danh sách) → nhập tiêu đề, chọn loại báo cáo (FRAUD_ANALYSIS / LOAN_ANALYSIS / THRESHOLD_RECOMMENDATION / SUPPRESSION_REVIEW / GENERAL) → submit → hệ thống lưu báo cáo với status PENDING_REVIEW và ghi AuditLog(ANALYST_REPORT_SUBMITTED) → MANAGER nhận thông báo và có thể xem xét báo cáo.

* *Tải báo cáo dạng PDF*: ANALYST (hoặc MANAGER) chọn một báo cáo → click tải PDF → hệ thống render Markdown thành PDF với header thương hiệu HPTRRĐGTC, bảng định dạng đầy đủ, tiếng Việt Unicode, và (nếu đã ACKNOWLEDGED) khối xác nhận màu xanh kèm tên MANAGER và ghi chú phản hồi → file PDF tải xuống ngay lập tức.
...
* *Xác nhận báo cáo ANALYST*: MANAGER xem danh sách báo cáo có status PENDING_REVIEW → chọn một báo cáo → đọc nội dung Markdown đầy đủ hoặc tải PDF → nhập ghi chú phản hồi (tùy chọn) → click Xác nhận → hệ thống chuyển status PENDING_REVIEW → ACKNOWLEDGED, ghi acknowledged_by, acknowledged_at, note và AuditLog(ANALYST_REPORT_ACKNOWLEDGED) → ANALYST có thể xem ghi chú phản hồi trong PDF được cập nhật.
...
* **Vai trò** (ROLE): mã vai trò (role_id), tên vai trò (role_name) — nhận một trong các giá trị: OPERATOR, REVIEWER, ANALYST, MANAGER, ADMIN
...
**Nhóm đối tượng liên quan đến Analyst Module:**

* **Cấu hình mô hình** (MODEL_CONFIG): mã cấu hình (config_id), tên mô hình (model_name) — fraud / loan, tên tham số (param_name), giá trị tham số (param_value), người cập nhật lần cuối (updated_by), thời điểm cập nhật (updated_at)
* **Suppression Rule** (SUPPRESSION_RULE): mã rule (rule_id), loại rule (rule_type) — MERCHANT / CUSTOMER / CARD, mã entity bị áp dụng (entity_id), lý do (reason), trạng thái (is_active), người tạo (created_by), thời điểm tạo (created_at), ngày hết hạn (expires_at)
* **Báo cáo Analyst** (ANALYST_REPORT): mã báo cáo (report_id), tiêu đề (title), loại báo cáo (report_type) — FRAUD_ANALYSIS / LOAN_ANALYSIS / THRESHOLD_RECOMMENDATION / SUPPRESSION_REVIEW / GENERAL, nội dung Markdown (content_md), trạng thái (status) — PENDING_REVIEW / ACKNOWLEDGED / ARCHIVED, người gửi (submitted_by), thời điểm gửi (submitted_at), người xác nhận (acknowledged_by), thời điểm xác nhận (acknowledged_at), ghi chú phản hồi (note)
...
* Một MODEL_CONFIG thuộc về đúng một mô hình (model_name = "fraud" hoặc "loan") và đại diện cho một tham số cụ thể. Mỗi lần ANALYST cập nhật ngưỡng, bản ghi MODEL_CONFIG tương ứng được ghi đè (không tạo lịch sử riêng — lịch sử được lưu trong AUDIT_LOG).

* Một SUPPRESSION_RULE áp dụng cho đúng một entity (merchant / customer / card). Một entity có thể có nhiều SUPPRESSION_RULE theo thời gian (ví dụ: rule cũ hết hạn, tạo rule mới). Tuy nhiên tại một thời điểm chỉ có tối đa một rule ACTIVE cho mỗi entity. Quan hệ 1–nhiều theo thời gian.

* Một ANALYST_REPORT được tạo bởi đúng một USER có role ANALYST (submitted_by). Một ANALYST có thể tạo nhiều báo cáo. Quan hệ nhiều-1 từ ANALYST_REPORT đến USER (submitter).

* Một ANALYST_REPORT có thể được xác nhận bởi tối đa một USER có role MANAGER/ADMIN (acknowledged_by). Báo cáo chưa xác nhận thì acknowledged_by = NULL. Quan hệ nhiều-0..1 từ ANALYST_REPORT đến USER (acknowledger).

* Mọi thay đổi ngưỡng (THRESHOLD_UPDATED), tạo/vô hiệu hóa suppression rule, submit/acknowledge báo cáo đều tạo bản ghi AUDIT_LOG tương ứng. Quan hệ: ANALYST_REPORT / SUPPRESSION_RULE / MODEL_CONFIG → nhiều AUDIT_LOG (1–nhiều).
