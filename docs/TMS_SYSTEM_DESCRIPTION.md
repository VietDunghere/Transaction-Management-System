...
* **MANAGER** (Quản lý):
  * Được thực hiện các chức năng như thành viên hệ thống
  * Xem Dashboard tổng quan hoạt động hệ thống theo ngày (số giao dịch, tỉ lệ phê duyệt, tỉ lệ gian lận, case tồn đọng)
  * Xem biểu đồ phân bố điểm gian lận (Fraud Score Chart)
  * Xem danh sách báo cáo giao dịch (lọc theo thời gian, trạng thái, kênh, khách hàng)
  * Xuất báo cáo giao dịch ra file CSV hoặc PDF
  * Xem Audit Log — lịch sử toàn bộ hành động nghiệp vụ trong hệ thống
  * Xem danh sách và chi tiết kết quả các lần chạy đối soát dữ liệu (Reconciliation Jobs)

* **ADMIN** (Kỹ thuật / Quản trị hệ thống):
  * Được thực hiện các chức năng như thành viên hệ thống
  * Tạo tài khoản người dùng mới, vô hiệu hóa tài khoản, cấp phát vai trò
  * Xem cấu trúc và thống kê các file thô trong Data Lake
  * Kích hoạt thủ công ETL Pipeline (nạp dữ liệu từ Data Lake vào Kho dữ liệu)
  * Xem log kết quả các lần chạy ETL
  * Kích hoạt và theo dõi Reconciliation Job (đối soát dữ liệu 3 nguồn)
...
* *Nộp giao dịch mới*: OPERATOR đăng nhập vào hệ thống → chọn chức năng nộp giao dịch → nhập thông tin giao dịch (mã khách hàng, số tiền, loại tiền tệ, tên đơn vị thụ hưởng/merchant, kênh giao dịch: ATM/POS/ONLINE) → hệ thống tính toán một chuỗi băm (txn_hash) từ các trường dữ liệu tĩnh của giao dịch để tạo Idempotency Key → hệ thống kiểm tra Idempotency Key này đã tồn tại trong bảng TXN_IDEMPOTENCY chưa:
  ...
  * Nếu **fraud_score ≤ 0.3**: giao dịch được **tự động phê duyệt** (APPROVED).
  * Nếu **fraud_score > 0.3 và ≤ 0.7** hoặc số tiền vượt ngưỡng quy định: giao dịch chuyển sang trạng thái **PENDING** và hệ thống tự động tạo một **REVIEW_CASE** để Reviewer xem xét thủ công.
  * Nếu **fraud_score > 0.7**: giao dịch bị **tự động từ chối** (REJECTED).
...
* *Nộp hồ sơ đề nghị vay vốn*: OPERATOR đăng nhập → chọn chức năng nộp hồ sơ vay → nhập thông tin tín dụng của khách hàng (mã khách hàng, số tiền đề nghị vay, mục đích vay, thu nhập hàng tháng, tỉ lệ nợ hiện tại / Debt-to-Income Ratio, lịch sử tín dụng) → hệ thống gọi mô hình AI Loan Risk Model để tính **PD Score** (Probability of Default — xác suất khách hàng không trả được nợ), nhận về giá trị từ 0.0 đến 1.0 → hệ thống phân loại mức độ rủi ro:
  * **LOW**: PD Score thấp — khả năng trả nợ tốt.
  * **MEDIUM**: PD Score trung bình — cần xem xét thêm điều kiện.
  * **HIGH**: PD Score cao — rủi ro không trả được nợ lớn.
...
* *Đưa ra quyết định phê duyệt/từ chối*: REVIEWER đã nhận case → đọc toàn bộ thông tin: thông tin khách hàng, chi tiết giao dịch, điểm gian lận, lịch sử giao dịch trước của khách hàng, lý do hệ thống tạo case → phân tích và đưa ra quyết định → gọi endpoint `PATCH /cases/{id}/decision` kèm: quyết định (APPROVED / REJECTED), ghi chú lý do nghiệp vụ (bắt buộc), và **số phiên bản case hiện tại** (version) → hệ thống thực hiện **Optimistic Locking**: kiểm tra `version` trong request có khớp với `version` trong DB không:
  * Nếu **khớp**: tiến hành xử lý → đóng case (status = CLOSED) → cập nhật trạng thái giao dịch tương ứng sang APPROVED hoặc REJECTED trong bảng TRANSACTIONS_LIVE → toàn bộ thay đổi được thực thi trong một **Database Transaction (COMMIT)** duy nhất để đảm bảo tính ACID → hệ thống ghi Audit Log gồm: ai quyết định, quyết định gì, lúc nào, ghi chú gì.
...
* *Xem cấu trúc Data Lake*: ADMIN chọn chức năng Data Lake → hệ thống gọi endpoint `/datalake/structure` → quét thư mục chứa file raw log (CSV/JSON) và trả về danh sách: cấu trúc thư mục theo ngày (YYYY/MM/DD), số lượng file, tổng dung lượng, ngày có dữ liệu mới nhất và cũ nhất → ADMIN dùng thông tin này để xác nhận Data Lake đang có đủ dữ liệu trước khi kích hoạt ETL.

* *Kích hoạt ETL Pipeline*: ADMIN chọn chức năng ETL → nhập khoảng ngày cần xử lý (ví dụ: 2025-01-01 đến 2025-01-31) → click Kích hoạt → hệ thống gọi endpoint `/etl/trigger` → ETL Pipeline bắt đầu chạy ngầm theo 3 giai đoạn:
...
* *Kích hoạt Reconciliation Job*: ADMIN chọn chức năng Reconciliation → nhập ngày cần đối soát → click Chạy đối soát → hệ thống gọi endpoint `/reconciliation/run` → Reconciliation Service thực hiện **đối soát 3 chiều (3-way match)**:
  1. Đếm số lượng và cộng tổng giá trị giao dịch ngày đó từ **OLTP** (bảng TRANSACTIONS_LIVE).
  2. Đếm số lượng và cộng tổng giá trị từ **Data Lake** (file raw log ngày đó).
  3. Đếm số lượng và cộng tổng giá trị từ **Data Warehouse** (bảng FACT_TRANSACTIONS).

  So sánh 3 bộ số liệu:
  * Nếu **MATCH** (cả 3 nguồn khớp 100%): ghi nhận trạng thái MATCH.
  * Nếu **MISMATCH** (ít nhất 1 nguồn lệch): ghi nhận trạng thái MISMATCH kèm chi tiết lệch → hệ thống gửi cảnh báo để điều tra ngay.
  * Nếu **FAILED** (lỗi kết nối DB hoặc timeout): ghi nhận trạng thái FAILED kèm thông tin lỗi.

  Kết quả ghi vào bảng RECONCILIATION_JOBS → ADMIN và MANAGER đều có thể xem kết quả.
...
* **Đơn vay** (LOAN_APPLICATION): mã đơn vay (loan_id), mã khách hàng (customer_id), số tiền đề nghị vay (requested_amount), mục đích vay (purpose), thu nhập hàng tháng (monthly_income), tỉ lệ nợ (debt_to_income_ratio), điểm xác suất vỡ nợ (pd_score), mức độ rủi ro (risk_level) — LOW / MEDIUM / HIGH, trạng thái đơn (status), thời điểm nộp (created_at)
* **Điểm rủi ro vay** (LOAN_RISK_SCORE): mã điểm (score_id), mã đơn vay (loan_id), điểm xác suất vỡ nợ (pd_score), phiên bản mô hình AI đã dùng (model_version), thời điểm chấm điểm (scored_at)

**Nhóm đối tượng liên quan đến xét duyệt thủ công:**

* **Hồ sơ xét duyệt** (REVIEW_CASE): mã case (case_id), mã giao dịch liên quan (txn_id), trạng thái case (status) — OPEN / ASSIGNED / CLOSED, mã Reviewer được gán (assigned_to), số phiên bản chống ghi đè (version), thời điểm mở case (opened_at), thời điểm đóng case (closed_at), ghi chú quyết định (decision_note)
...
* Một USER có thể được gán nhiều ROLE (ví dụ: một người vừa là OPERATOR vừa là REVIEWER trong môi trường nhỏ). Một ROLE có thể được gán cho nhiều USER. Quan hệ nhiều-nhiều thông qua bảng trung gian USER_ROLE.
