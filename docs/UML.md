...
| UC02.2 | Xem danh sách giao dịch | OPERATOR, REVIEWER, MANAGER, ADMIN, ANALYST |
| UC02.3 | Xem chi tiết giao dịch | OPERATOR, REVIEWER, MANAGER, ADMIN, ANALYST |
| UC03.1 | Nộp hồ sơ đề nghị vay vốn | OPERATOR |
| UC03.2 | Mô phỏng chấm điểm rủi ro vay (Simulate) | OPERATOR, MANAGER, ADMIN, ANALYST |
| UC03.3 | Xem danh sách hồ sơ vay | OPERATOR, MANAGER, ADMIN, ANALYST |
| UC03.4 | Xem chi tiết hồ sơ vay | OPERATOR, MANAGER, ADMIN, ANALYST |
...
| UC01.3 | Đổi mật khẩu cá nhân | Thành viên hệ thống | — |
| UC02.1 | Nộp giao dịch mới | OPERATOR | — |
| UC02.2 | Xem danh sách giao dịch | OPERATOR, REVIEWER, MANAGER, ADMIN, ANALYST | UC02.3 <<extend>> UC02.2 |
| UC02.3 | Xem chi tiết giao dịch | OPERATOR, REVIEWER, MANAGER, ADMIN, ANALYST | <<extend>> UC02.2 |
| UC03.1 | Nộp hồ sơ đề nghị vay vốn | OPERATOR | — |
| UC03.2 | Mô phỏng chấm điểm rủi ro vay (Simulate) | OPERATOR, MANAGER, ADMIN, ANALYST | — |
| UC03.3 | Xem danh sách hồ sơ vay | OPERATOR, MANAGER, ADMIN, ANALYST | UC03.4 <<extend>> UC03.3 |
| UC03.4 | Xem chi tiết hồ sơ vay | OPERATOR, MANAGER, ADMIN, ANALYST | <<extend>> UC03.3 |
| UC03.5 | Phê duyệt / Từ chối hồ sơ vay | MANAGER, ADMIN | — |
| UC04.1 | Xem danh sách case chờ xử lý | REVIEWER, ANALYST, MANAGER, ADMIN | UC04.3 <<extend>> UC04.1 |
| UC04.2 | Nhận case (Assign) | REVIEWER | — |
| UC04.3 | Xem chi tiết case | REVIEWER, ANALYST, MANAGER, ADMIN | <<extend>> UC04.1 |
| UC04.4 | Ra quyết định Phê duyệt / Từ chối | REVIEWER, MANAGER, ADMIN | — |
...
| REVIEWER | Cụ thể | Thành viên hệ thống | Nhân viên xét duyệt case thủ công — tiếp nhận và ra quyết định các giao dịch MANUAL_REVIEW |
...
| MANAGER | Cụ thể | Thành viên hệ thống | Quản lý theo dõi hoạt động, báo cáo, phê duyệt/từ chối hồ sơ vay và xác nhận báo cáo phân tích |
...
| UC02.2 | Xem danh sách giao dịch | OPERATOR, REVIEWER, MANAGER, ADMIN, ANALYST | UC này cho phép người dùng xem danh sách toàn bộ giao dịch trong hệ thống với filter theo status, ngày, merchant, số tiền |
| UC02.3 | Xem chi tiết giao dịch | OPERATOR, REVIEWER, MANAGER, ADMIN, ANALYST | UC này cho phép người dùng xem toàn bộ thông tin của một giao dịch: fraud_score, trạng thái, lý do phân loại, và mã case liên quan nếu có |
...
| UC03.2 | Mô phỏng chấm điểm rủi ro vay (Simulate) | OPERATOR, MANAGER, ADMIN, ANALYST | UC này cho phép người dùng nhập các trường AI để nhận PD Score và phân loại rủi ro tức thì mà không lưu vào DB — dùng để kiểm tra trước khi nộp đơn chính thức |
| UC03.3 | Xem danh sách hồ sơ vay | OPERATOR, MANAGER, ADMIN, ANALYST | UC này cho phép xem danh sách toàn bộ hồ sơ vay, filter theo trạng thái và customer |
| UC03.4 | Xem chi tiết hồ sơ vay | OPERATOR, MANAGER, ADMIN, ANALYST | UC này cho phép xem toàn bộ thông tin hồ sơ vay kèm kết quả PD Score, phân loại rủi ro, và trạng thái xét duyệt |
...
| Mã UC | Use Case | Actor | Mô tả |
|-------|----------|-------|-------|
| UC04.1 | Xem danh sách case chờ xử lý | REVIEWER, ANALYST, MANAGER, ADMIN | UC này cho phép xem danh sách các case đang ở trạng thái OPEN (chưa ai nhận) hoặc ASSIGNED (đã nhận nhưng chưa quyết định). ANALYST chỉ đọc — không thể assign hay quyết định |
...
| UC04.3 | Xem chi tiết case | REVIEWER, ANALYST, MANAGER, ADMIN | UC này cho phép xem toàn bộ thông tin của một case: thông tin giao dịch liên quan, fraud_score, lịch sử trạng thái và người phụ trách |
...
| Use Case con | Actor | Quan hệ | Mô tả |
|---|---|---|---|
| Lọc danh sách giao dịch | OPERATOR, REVIEWER, MANAGER, ADMIN, ANALYST | `<<extend>>` từ Xem danh sách giao dịch | UC này cho phép người dùng thu hẹp kết quả theo: khoảng thời gian, trạng thái, kênh giao dịch (ATM / POS / ONLINE), ngưỡng fraud_score |
...
#### Chi tiết: Mô phỏng chấm điểm rủi ro vay (OPERATOR, MANAGER, ADMIN, ANALYST)
...
| Use Case con | Actor | Quan hệ | Mô tả |
|---|---|---|---|
| Điền đầy đủ thông tin mô phỏng | OPERATOR, MANAGER, ADMIN, ANALYST | `<<include>>` từ Mô phỏng | UC này cho phép người dùng nhập tất cả trường AI bắt buộc: person_age, person_income, person_home_ownership, person_emp_length, loan_intent, loan_grade, loan_amnt, loan_int_rate, cb_person_default_on_file, cb_person_cred_hist_length |
| Xem kết quả mô phỏng PD Score | OPERATOR, MANAGER, ADMIN, ANALYST | `<<include>>` từ Mô phỏng | UC này cho phép người dùng nhận ngay: pd_score (0.0–1.0), risk_level (LOW/MEDIUM/HIGH), top_risk_factors, model_version — không lưu vào DB |
...
| Use Case con | Actor | Quan hệ | Mô tả |
|---|---|---|---|
| Lọc danh sách hồ sơ vay | OPERATOR, MANAGER, ADMIN, ANALYST | `<<extend>>` từ Xem danh sách hồ sơ vay | UC này cho phép người dùng lọc hồ sơ vay theo trạng thái, mức rủi ro (LOW / MEDIUM / HIGH), khoảng thời gian nộp. OPERATOR chỉ thấy đơn của mình |
