...
| UC02.2 | Xem giao dịch | REVIEWER, ANALYST |
...
| 2 | UC03.3 – Xem danh sách hồ sơ vay | Xem chi tiết hồ sơ vay | **UC03.3 Xem hồ sơ vay** |
| 3 | UC04.1 – Xem danh sách case chờ xử lý | Xem chi tiết case | **UC04.1 Xem case** |
...
| UC05.4 | Xuất báo cáo (CSV / PDF) | ANALYST, MANAGER, ADMIN | <<include>> UC05.3 |
...
**Kết quả:** 44 UC nghiệp vụ + 1 UC trừu tượng (UC06.A), tổ chức thành **8 nhóm UC**, liên kết với **6 actor** (1 trừu tượng + 5 cụ thể), 6 cặp quan hệ <<extend>>, 3 quan hệ <<include>>, 1 cặp generalization UC.
...
| UC02.2 | Xem giao dịch | REVIEWER, ANALYST | UC này cho phép người dùng xem danh sách giao dịch (filter theo status, ngày, merchant, fraud_score) và xem chi tiết từng giao dịch: fraud_score, trạng thái, lý do phân loại, mã case liên quan |
...
| Use Case con | Actor | Quan hệ | Mô tả |
|---|---|---|---|
| Lọc danh sách giao dịch | REVIEWER, ANALYST | `<<extend>>` từ Xem giao dịch | UC này cho phép người dùng thu hẹp kết quả theo: khoảng thời gian, trạng thái, kênh giao dịch (ATM / POS / ONLINE), ngưỡng fraud_score |
