...
| UC02.2 | Xem danh sách giao dịch | OPERATOR, REVIEWER, MANAGER |
| UC02.3 | Xem chi tiết giao dịch | OPERATOR, REVIEWER, MANAGER |
| UC03.1 | Nộp hồ sơ đề nghị vay vốn | OPERATOR |
| UC03.2 | Mô phỏng chấm điểm rủi ro vay (Simulate) | OPERATOR, REVIEWER, MANAGER |
| UC03.3 | Xem danh sách hồ sơ vay | OPERATOR, REVIEWER, MANAGER |
| UC03.4 | Xem chi tiết hồ sơ vay | OPERATOR, REVIEWER, MANAGER |
| UC03.5 | Phê duyệt / Từ chối hồ sơ vay | REVIEWER, MANAGER |
...
| UC05.1 | Xem Dashboard tổng quan | ANALYST, MANAGER, ADMIN |
| UC05.2 | Xem biểu đồ phân bố Fraud Score | ANALYST, MANAGER, ADMIN |
| UC05.3 | Xem báo cáo giao dịch | ANALYST, MANAGER, ADMIN |
| UC05.4 | Xuất báo cáo (CSV / PDF) | ANALYST, MANAGER, ADMIN |
| UC05.5 | Xem Audit Log hệ thống | ANALYST, MANAGER, ADMIN |
| UC05.6 | Truy vết lịch sử một giao dịch cụ thể | ANALYST, MANAGER, ADMIN |
...
| UC08.1 | Xem ngưỡng phân loại hiện hành | ANALYST, MANAGER, ADMIN |
| UC08.2 | Cập nhật ngưỡng Fraud Score | ANALYST |
| UC08.3 | Cập nhật ngưỡng PD Score | ANALYST |
| UC08.4 | Xem hiệu suất mô hình Fraud | ANALYST, MANAGER, ADMIN |
| UC08.5 | Xem hiệu suất mô hình PD Score | ANALYST, MANAGER, ADMIN |
| UC08.6 | Xem danh sách Suppression Rules | ANALYST, MANAGER, ADMIN |
| UC08.7 | Tạo Suppression Rule mới | ANALYST |
| UC08.8 | Vô hiệu hóa Suppression Rule | ANALYST |
| UC08.9 | Tạo báo cáo phân tích | ANALYST |
| UC08.10 | Xem danh sách báo cáo phân tích | ANALYST, MANAGER, ADMIN |
| UC08.11 | Xem chi tiết và tải PDF báo cáo | ANALYST, MANAGER, ADMIN |
| UC08.12 | Xác nhận (Acknowledge) báo cáo | MANAGER, ADMIN |
...
**Phân tích nhóm ngưỡng (UC08.1–08.3):** UC08.1 là UC xem chung cho cả hai loại ngưỡng, UC08.2/08.3 là các UC cập nhật chuyên biệt. Actor của UC08.1 rộng hơn (ANALYST, MANAGER, ADMIN) so với UC08.2/08.3 (chỉ ANALYST). Không gộp vì ngữ nghĩa khác nhau: xem là read-only cho nhiều role; cập nhật là write chỉ dành cho chuyên viên.
...
| UC01.3 | Đổi mật khẩu cá nhân | Thành viên hệ thống | — |
| UC02.1 | Nộp giao dịch mới | OPERATOR | — |
| UC02.2 | Xem danh sách giao dịch | OPERATOR, REVIEWER, MANAGER | UC02.3 <<extend>> UC02.2 |
| UC02.3 | Xem chi tiết giao dịch | OPERATOR, REVIEWER, MANAGER | <<extend>> UC02.2 |
| UC03.1 | Nộp hồ sơ đề nghị vay vốn | OPERATOR | — |
| UC03.2 | Mô phỏng chấm điểm rủi ro vay (Simulate) | OPERATOR, REVIEWER, MANAGER | — |
| UC03.3 | Xem danh sách hồ sơ vay | OPERATOR, REVIEWER, MANAGER | UC03.4 <<extend>> UC03.3 |
| UC03.4 | Xem chi tiết hồ sơ vay | OPERATOR, REVIEWER, MANAGER | <<extend>> UC03.3 |
| UC03.5 | Phê duyệt / Từ chối hồ sơ vay | REVIEWER, MANAGER | — |
...
| UC04.4 | Ra quyết định Phê duyệt / Từ chối | REVIEWER, MANAGER, ADMIN | — |
| UC05.1 | Xem Dashboard tổng quan | ANALYST, MANAGER, ADMIN | UC05.2 <<extend>> UC05.1 |
| UC05.2 | Xem biểu đồ xu hướng Fraud (fraud-trend) | ANALYST, MANAGER, ADMIN | <<extend>> UC05.1 |
| UC05.3 | Xem báo cáo giao dịch | ANALYST, MANAGER, ADMIN | UC05.4 <<include>> UC05.3 |
| UC05.4 | Xuất báo cáo (CSV / JSON) | ANALYST, MANAGER, ADMIN | <<include>> UC05.3 |
| UC05.5 | Xem Audit Log hệ thống | ANALYST, MANAGER, ADMIN | UC05.6 <<extend>> UC05.5 |
| UC05.6 | Truy vết lịch sử một giao dịch cụ thể | ANALYST, MANAGER, ADMIN | <<extend>> UC05.5 |
| UC05.7 | Xem kết quả đối soát (Reconciliation) | ANALYST, ADMIN | — |
...
| UC08.1 | Xem ngưỡng phân loại hiện hành | ANALYST, MANAGER, ADMIN | UC08.2 <<include>> UC08.1; UC08.3 <<include>> UC08.1 |
...
| UC08.8 | Vô hiệu hóa Suppression Rule | ANALYST | — |
| UC08.9 | Tạo báo cáo phân tích | ANALYST | — |
| UC08.10 | Xem danh sách báo cáo phân tích | ANALYST, MANAGER, ADMIN | UC08.11 <<extend>> UC08.10 |
| UC08.11 | Xem chi tiết và tải PDF báo cáo | ANALYST, MANAGER, ADMIN | <<extend>> UC08.10 |
| UC08.12 | Xác nhận (Acknowledge) báo cáo | MANAGER, ADMIN | — |
...
| UC02.2 | Xem danh sách giao dịch | OPERATOR, REVIEWER, MANAGER | UC này cho phép người dùng xem danh sách toàn bộ giao dịch trong hệ thống với filter theo status, ngày, merchant, số tiền |
| UC02.3 | Xem chi tiết giao dịch | OPERATOR, REVIEWER, MANAGER | UC này cho phép người dùng xem toàn bộ thông tin của một giao dịch: fraud_score, trạng thái, lý do phân loại, và mã case liên quan nếu có |
...
| UC03.2 | Mô phỏng chấm điểm rủi ro vay (Simulate) | OPERATOR, REVIEWER, MANAGER | UC này cho phép người dùng nhập các trường AI để nhận PD Score và phân loại rủi ro tức thì mà không lưu vào DB — dùng để kiểm tra trước khi nộp đơn chính thức |
| UC03.3 | Xem danh sách hồ sơ vay | OPERATOR, REVIEWER, MANAGER | UC này cho phép xem danh sách toàn bộ hồ sơ vay, filter theo trạng thái và customer |
| UC03.4 | Xem chi tiết hồ sơ vay | OPERATOR, REVIEWER, MANAGER | UC này cho phép xem toàn bộ thông tin hồ sơ vay kèm kết quả PD Score, phân loại rủi ro, và trạng thái xét duyệt |
| UC03.5 | Phê duyệt / Từ chối hồ sơ vay | REVIEWER, MANAGER | UC này cho phép REVIEWER đưa ra quyết định APPROVE hoặc REJECT cho đơn vay đang PENDING. MANAGER có thể override. Ràng buộc SoD: người phê duyệt không được là người đã tạo đơn (4-eyes principle) |
...
| Mã UC | Use Case | Actor | Mô tả |
|-------|----------|-------|-------|
| UC05.1 | Xem Dashboard tổng quan | ANALYST, MANAGER, ADMIN | UC này cho phép xem tổng hợp số liệu KPI hoạt động hệ thống (tổng giao dịch, tỉ lệ phê duyệt, tỉ lệ gian lận, case tồn đọng) theo khoảng thời gian |
| UC05.2 | Xem biểu đồ phân bố Fraud Score | ANALYST, MANAGER, ADMIN | UC này cho phép xem biểu đồ histogram phân bố fraud_score của các giao dịch theo dải điểm, để đánh giá hiệu quả mô hình AI |
| UC05.3 | Xem báo cáo giao dịch | ANALYST, MANAGER, ADMIN | UC này cho phép tra cứu danh sách giao dịch với các điều kiện lọc: khoảng thời gian, trạng thái, kênh, ngưỡng fraud_score |
| UC05.4 | Xuất báo cáo (CSV / PDF) | ANALYST, MANAGER, ADMIN | UC này cho phép xuất toàn bộ dữ liệu báo cáo đang xem ra file CSV hoặc PDF để lưu trữ và báo cáo kiểm toán |
| UC05.5 | Xem Audit Log hệ thống | ANALYST, MANAGER, ADMIN | UC này cho phép xem toàn bộ nhật ký hành động nghiệp vụ trong hệ thống (actor, hành động, đối tượng bị tác động, giá trị trước/sau) |
| UC05.6 | Truy vết lịch sử một giao dịch cụ thể | ANALYST, MANAGER, ADMIN | UC này cho phép xem toàn bộ timeline trạng thái của một giao dịch: khi nào tạo, ai xét duyệt, lý do quyết định, trạng thái thay đổi như thế nào |
...
| Mã UC | Use Case | Actor | Mô tả |
|-------|----------|-------|-------|
| UC08.1 | Xem ngưỡng phân loại hiện hành | ANALYST, MANAGER, ADMIN | UC này cho phép xem các ngưỡng phân loại đang áp dụng cho mô hình Fraud (reject_threshold, review_threshold) và mô hình PD Score (risk_high_threshold, risk_medium_threshold) |
...
| UC08.4 | Xem hiệu suất mô hình Fraud | ANALYST, MANAGER, ADMIN | UC này cho phép xem các chỉ số đánh giá mô hình Fraud: precision, recall, F1-score, AUC-ROC, phân phối fraud_score theo khoảng, và trend theo thời gian |
| UC08.5 | Xem hiệu suất mô hình PD Score | ANALYST, MANAGER, ADMIN | UC này cho phép xem các chỉ số đánh giá mô hình PD Score: phân bố pd_score, tỉ lệ default theo risk_level, so sánh ngưỡng hiện hành với phân phối thực tế |
| UC08.6 | Xem danh sách Suppression Rules | ANALYST, MANAGER, ADMIN | UC này cho phép xem tất cả suppression rules (active và inactive): điều kiện lọc, ngưỡng, trạng thái, ngày tạo và người tạo |
...
| UC08.10 | Xem danh sách báo cáo phân tích | ANALYST, MANAGER, ADMIN | UC này cho phép xem danh sách các báo cáo đã tạo: tiêu đề, tác giả, kỳ báo cáo, trạng thái (DRAFT / PUBLISHED / ACKNOWLEDGED) |
| UC08.11 | Xem chi tiết và tải PDF báo cáo | ANALYST, MANAGER, ADMIN | UC này cho phép xem nội dung đầy đủ của báo cáo và tải file PDF để lưu trữ hoặc trình lãnh đạo |
| UC08.12 | Xác nhận (Acknowledge) báo cáo | MANAGER, ADMIN | UC này cho phép MANAGER/ADMIN xác nhận đã đọc và chấp thuận nội dung báo cáo phân tích, chuyển trạng thái báo cáo từ PUBLISHED sang ACKNOWLEDGED |
...
| Use Case con | Actor | Quan hệ | Mô tả |
|---|---|---|---|
| Lọc danh sách giao dịch | OPERATOR, REVIEWER, MANAGER | `<<extend>>` từ Xem danh sách giao dịch | UC này cho phép người dùng thu hẹp kết quả theo: khoảng thời gian, trạng thái, kênh giao dịch (ATM / POS / ONLINE), ngưỡng fraud_score |
...
#### Chi tiết: Mô phỏng chấm điểm rủi ro vay (OPERATOR, REVIEWER, MANAGER)
...
| Use Case con | Actor | Quan hệ | Mô tả |
|---|---|---|---|
| Điền đầy đủ thông tin mô phỏng | OPERATOR, REVIEWER, MANAGER | `<<include>>` từ Mô phỏng | UC này cho phép người dùng nhập tất cả trường AI bắt buộc: person_age, person_income, person_home_ownership, person_emp_length, loan_intent, loan_grade, loan_amnt, loan_int_rate, cb_person_default_on_file, cb_person_cred_hist_length |
| Xem kết quả mô phỏng PD Score | OPERATOR, REVIEWER, MANAGER | `<<include>>` từ Mô phỏng | UC này cho phép người dùng nhận ngay: pd_score (0.0–1.0), risk_level (LOW/MEDIUM/HIGH), top_risk_factors, model_version — không lưu vào DB |
...
| Use Case con | Actor | Quan hệ | Mô tả |
|---|---|---|---|
| Lọc danh sách hồ sơ vay | OPERATOR, REVIEWER, MANAGER | `<<extend>>` từ Xem danh sách hồ sơ vay | UC này cho phép người dùng lọc hồ sơ vay theo trạng thái, mức rủi ro (LOW / MEDIUM / HIGH), khoảng thời gian nộp |

---

#### Chi tiết: Phê duyệt / Từ chối hồ sơ vay (REVIEWER, MANAGER)
...
| Use Case con | Actor | Quan hệ | Mô tả |
|---|---|---|---|
| Nhập quyết định và ghi chú | REVIEWER, MANAGER | `<<include>>` từ Phê duyệt/Từ chối | UC này cho phép người dùng chọn APPROVE hoặc REJECT, nhập review_note (tùy chọn) và version hiện tại của đơn |
| Xác nhận quyết định | REVIEWER, MANAGER | `<<include>>` từ Phê duyệt/Từ chối | UC này cho phép người dùng xác nhận gửi quyết định. Nếu APPROVE: hệ thống tự tính monthly_payment, outstanding_balance, maturity_date |
| Tải lại đơn vay | REVIEWER, MANAGER | `<<extend>>` từ Phê duyệt/Từ chối | UC này cho phép người dùng tải lại dữ liệu đơn khi hệ thống trả 409 Conflict do version không khớp |
...
| Use Case con | Actor | Quan hệ | Mô tả |
|---|---|---|---|
| Xem ngưỡng phân loại hiện hành | ANALYST, MANAGER, ADMIN | UC08.1 | UC này cho phép xem reject_threshold, review_threshold (Fraud) và risk_high_threshold, risk_medium_threshold (PD Score) đang áp dụng |
...
| Use Case con | Actor | Quan hệ | Mô tả |
|---|---|---|---|
| Xem metrics Fraud model | ANALYST, MANAGER, ADMIN | `<<include>>` từ UC08.4 | UC này cho phép xem: precision, recall, F1-score, AUC-ROC, tỉ lệ False Positive/Negative, phân phối fraud_score theo khoảng |
| Xem metrics PD Score model | ANALYST, MANAGER, ADMIN | `<<include>>` từ UC08.5 | UC này cho phép xem: phân bố pd_score, tỉ lệ default theo risk_level (LOW/MEDIUM/HIGH), Gini coefficient, so sánh với ngưỡng hiện tại |
| Lọc theo khoảng thời gian | ANALYST, MANAGER, ADMIN | `<<extend>>` từ UC08.4/UC08.5 | UC này cho phép chọn khoảng ngày phân tích (7 ngày / 30 ngày / 90 ngày / custom) |
