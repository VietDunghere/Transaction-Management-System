...
#### Nhóm 5 — UC05.4 "Xuất báo cáo" phụ thuộc bắt buộc vào UC02.2 "Xem giao dịch"

**Phân tích:** Người dùng **phải đang ở màn hình danh sách giao dịch** (UC02.2) thì nút "Xuất file" mới xuất hiện và có thể bấm. Không thể xuất nếu chưa tra cứu và hiển thị dữ liệu. Đây là quan hệ **bắt buộc, không có ngoại lệ**.

**Quyết định: UC05.4 Xuất báo cáo `<<include>>` UC02.2 Xem giao dịch.**
...
| UC05.4 | Xuất báo cáo (CSV / PDF) | ANALYST, MANAGER, ADMIN | <<include>> UC02.2 |
...
**Kết quả:** 40 UC nghiệp vụ + 1 UC trừu tượng (UC06.A), tổ chức thành **8 nhóm UC**, liên kết với **6 actor** (1 trừu tượng + 5 cụ thể), 6 cặp quan hệ <<extend>>, 3 quan hệ <<include>>, 1 cặp generalization UC.
...
4. Tùy chọn: click "Xuất file" → chọn định dạng → tải file (UC05.4)
...
#### Chi tiết: Xuất báo cáo (CSV / PDF) (ANALYST, MANAGER, ADMIN)

Người dùng đang ở màn hình "Xem giao dịch" (UC02.2) → click "Xuất file" → chọn định dạng → tải file.

**Quan hệ use case con:**
- **Xuất báo cáo (CSV / PDF)** `<<include>>` **Xem giao dịch** (UC02.2) — phải có danh sách kết quả trước
