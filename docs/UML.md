...
| UC02.2 | Xem giao dịch | ANALYST, MANAGER, ADMIN |
| UC03.1 | Nộp hồ sơ đề nghị vay vốn | OPERATOR |
| UC03.2 | Mô phỏng chấm điểm rủi ro vay (Simulate) | OPERATOR, REVIEWER |
| UC03.3 | Xem hồ sơ vay | OPERATOR, REVIEWER |
| UC03.4 | Phê duyệt / Từ chối hồ sơ vay | REVIEWER |
| UC04.1 | Xem case | REVIEWER, MANAGER |
| UC04.2 | Nhận case (Assign) | REVIEWER |
| UC04.3 | Ra quyết định Phê duyệt / Từ chối | REVIEWER, MANAGER |
| UC05.1 | Xem Dashboard tổng quan | ANALYST, MANAGER |
| UC05.2 | Xem biểu đồ phân bố Fraud Score | ANALYST, MANAGER |
| UC05.3 | Xuất báo cáo (CSV / PDF) | MANAGER, ADMIN |
| UC05.4 | Xem Audit Log hệ thống | MANAGER, ADMIN |
| UC05.5 | Truy vết lịch sử một giao dịch cụ thể | MANAGER, ADMIN |
...
| UC06.5 | Xem người dùng | MANAGER, ADMIN |
| UC07.1 | Xem danh sách Data Lake Snapshots | ADMIN |
| UC07.2 | Kích hoạt ETL Pipeline | ADMIN |
| UC07.3 | Xem ETL Logs | ADMIN |
| UC07.4 | Kích hoạt Reconciliation Job | ADMIN |
| UC07.5 | Xem kết quả Reconciliation | ADMIN |
...
#### Nhóm 1 — Cặp "Xem danh sách" và "Xem chi tiết" (4 cặp)

Trong hệ thống HPTRRĐGTC, 4 cặp UC sau đây có **thao tác giống nhau**: người dùng browse danh sách → click một dòng để xem chi tiết.

| Cặp | "Xem danh sách" (cũ) | "Xem chi tiết" (cũ) | UC sau gộp |
|-----|---------------------|---------------------|------------|
| 1 | UC02.2 – Xem danh sách giao dịch | UC02.3 – Xem chi tiết giao dịch | **UC02.2 Xem giao dịch** |
| 2 | UC03.3 – Xem danh sách hồ sơ vay | UC03.4 – Xem chi tiết hồ sơ vay | **UC03.3 Xem hồ sơ vay** |
| 3 | UC04.1 – Xem danh sách case chờ xử lý | UC04.3 – Xem chi tiết case | **UC04.1 Xem case** |
| 4 | UC06.5 – Xem danh sách người dùng | UC06.6 – Xem chi tiết thông tin người dùng | **UC06.5 Xem người dùng** |


**Phân tích — có thể gộp không?**

Xét từng cặp: người dùng **không thể xem chi tiết mà không đi qua danh sách** — đây là 1 flow liên tục (vào danh sách → lọc → click 1 dòng → xem chi tiết). Actor của từng cặp **giống nhau hoàn toàn**. Không có UC nào cho phép truy cập trực tiếp màn hình "chi tiết" mà bỏ qua danh sách.

**Quyết định: Gộp 4 cặp thành 4 UC đơn.** Mỗi UC bao phủ toàn bộ flow list→detail. Tên rút gọn: bỏ "danh sách" / "chi tiết", giữ lại tên đối tượng.
...
|-----------------|--------------------------|
| UC02.2 Xem giao dịch | Lọc theo trạng thái, kênh, khoảng fraud_score, thời gian |
| UC03.3 Xem hồ sơ vay | Lọc theo trạng thái, mức rủi ro (LOW/MEDIUM/HIGH), thời gian |
| UC04.1 Xem case | Lọc theo khoảng fraud_score, sắp xếp theo thời gian chờ |
| UC06.5 Xem người dùng | Lọc theo vai trò, trạng thái tài khoản |
| UC05.4 Xem Audit Log hệ thống | Lọc theo actor, loại hành động, thời gian |
...
#### Nhóm 4 — UC07.2 "Kích hoạt ETL Pipeline" và UC07.4 "Kích hoạt Reconciliation Job"
...
Cả hai đều chỉ ADMIN thực thi và xem kết quả — không có lý do phân tách actor. Không gộp vì ngữ nghĩa nghiệp vụ khác nhau, không vì actor khác nhau.
...
#### Nhóm 5 — UC05.3 "Xuất báo cáo" là mở rộng tùy chọn từ UC02.2 "Xem giao dịch"

**Phân tích:** Nút "Xuất file" chỉ xuất hiện khi người dùng đang ở màn hình UC02.2. Export là hành động **tùy chọn** — không phải mọi lần xem giao dịch đều export. Đây là quan hệ có điều kiện, kích hoạt từ base UC.

**Quyết định: UC05.3 Xuất báo cáo `<<extend>>` UC02.2 Xem giao dịch.**
...
- Dẫn đến **luồng nghiệp vụ khác nhau** (fraud routing tức thì vs loan workflow PENDING → REVIEWER duyệt)
...
#### Nhóm 8 — UC03.4 "Phê duyệt / Từ chối hồ sơ vay" và UC04.3 "Ra quyết định Phê duyệt / Từ chối" (case)

Cả hai UC cùng thao tác "chọn APPROVE/REJECT → nhập ghi chú → xác nhận", actor chung là REVIEWER.

**Phân tích:**
- **Đối tượng khác nhau**: UC03.4 tác động lên Loan Application; UC04.3 tác động lên ReviewCase + Transaction
- **Ràng buộc khác nhau**: UC03.4 có SoD check (submitter ≠ approver); UC04.3 có ASSIGNED status check + ownership check cho REVIEWER
...
**Quyết định: Giữ nguyên cấu trúc 13 UC trong UC08.**
...
| UC05.2 | Xem biểu đồ phân bố Fraud Score | ANALYST, MANAGER | <<extend>> UC05.1 |
| UC05.3 | Xuất báo cáo (CSV / PDF) | MANAGER, ADMIN | <<extend>> UC02.2 |
| UC05.4 | Xem Audit Log hệ thống | MANAGER, ADMIN | UC05.5 <<extend>> UC05.4 |
| UC05.5 | Truy vết lịch sử một giao dịch cụ thể | MANAGER, ADMIN | <<extend>> UC05.4 |
...
**Kết quả:** 40 UC nghiệp vụ + 1 UC trừu tượng (UC06.A), tổ chức thành **8 nhóm UC**, liên kết với **6 actor** (1 trừu tượng + 5 cụ thể), 3 cặp quan hệ <<extend>>, 2 quan hệ <<include>>, 1 cặp generalization UC.
...
| UC02.2 | Xem giao dịch | ANALYST, MANAGER, ADMIN | UC này cho phép xem toàn bộ danh sách giao dịch (filter theo status, ngày, merchant, fraud_score) và xem chi tiết từng giao dịch: fraud_score, trạng thái, lý do phân loại, mã case liên quan. REVIEWER không có quyền truy cập UC này |
...
| Mã UC | Use Case | Actor | Mô tả |
|-------|----------|-------|-------|
| UC03.1 | Nộp hồ sơ đề nghị vay vốn | OPERATOR | UC này cho phép Core Banking System của ngân hàng đối tác nộp đơn vay của khách hàng vào HPTRRĐGTC. Hệ thống tính PD Score và phân loại mức rủi ro (LOW/MEDIUM/HIGH). Đơn được lưu với trạng thái PENDING chờ REVIEWER duyệt. Chỉ OPERATOR mới được nộp |
...
| UC03.3 | Xem hồ sơ vay | OPERATOR, REVIEWER | UC này cho phép xem danh sách toàn bộ hồ sơ vay (filter theo trạng thái và customer) và xem chi tiết từng hồ sơ: PD Score, phân loại rủi ro, trạng thái xét duyệt |
| UC03.4 | Phê duyệt / Từ chối hồ sơ vay | REVIEWER | UC này cho phép REVIEWER đưa ra quyết định APPROVE hoặc REJECT cho đơn vay đang PENDING. Ràng buộc SoD: người phê duyệt không được là người đã tạo đơn (4-eyes principle) |
...
| Mã UC | Use Case | Actor | Mô tả |
|-------|----------|-------|-------|
| UC04.1 | Xem case | REVIEWER, MANAGER | UC này cho phép xem danh sách các case (OPEN hoặc ASSIGNED) và xem chi tiết từng case: giao dịch liên quan, fraud_score, lịch sử trạng thái, người phụ trách |
...
| UC04.3 | Ra quyết định Phê duyệt / Từ chối | REVIEWER, MANAGER | UC này cho phép đưa ra quyết định cuối cùng (APPROVED / REJECTED) cho case. REVIEWER chỉ quyết định được case đã nhận về mình; MANAGER có thể quyết định bất kỳ case ASSIGNED nào (override/giám sát). Case phải ở trạng thái ASSIGNED, không thể quyết định case OPEN trực tiếp |
...
| Mã UC | Use Case | Actor | Mô tả |
|-------|----------|-------|-------|
| UC05.1 | Xem Dashboard tổng quan | ANALYST, MANAGER | UC này cho phép xem tổng hợp số liệu KPI hoạt động hệ thống (tổng giao dịch, tỉ lệ phê duyệt, tỉ lệ gian lận, case tồn đọng) theo khoảng thời gian |
...
| UC05.3 | Xuất báo cáo (CSV / PDF) | MANAGER, ADMIN | UC này cho phép xuất toàn bộ dữ liệu báo cáo đang xem ra file CSV hoặc PDF để lưu trữ và báo cáo kiểm toán |
| UC05.4 | Xem Audit Log hệ thống | MANAGER, ADMIN | UC này cho phép xem toàn bộ nhật ký hành động nghiệp vụ trong hệ thống (actor, hành động, đối tượng bị tác động, giá trị trước/sau) |
| UC05.5 | Truy vết lịch sử một giao dịch cụ thể | MANAGER, ADMIN | UC này cho phép xem toàn bộ timeline trạng thái của một giao dịch: khi nào tạo, ai xét duyệt, lý do quyết định, trạng thái thay đổi như thế nào |
...
| UC06.5 | Xem người dùng | MANAGER, ADMIN | UC này cho phép xem danh sách toàn bộ tài khoản người dùng và xem chi tiết từng tài khoản: vai trò, trạng thái, email, thời gian tạo (MANAGER chỉ đọc, ADMIN có thể thao tác) |
...
| Mã UC | Use Case | Actor | Mô tả |
|-------|----------|-------|-------|
| UC07.1 | Xem danh sách Data Lake Snapshots | ADMIN | UC này cho phép ADMIN xem danh sách các snapshots trong Data Lake: loại snapshot, ngày, số records, tổng amount — để xác nhận dữ liệu trước khi chạy ETL |
...
| UC07.3 | Xem ETL Logs | ADMIN | UC này cho phép ADMIN xem danh sách và chi tiết các lần ETL đã chạy: trạng thái (SUCCESS / PARTIAL / FAILED), số bản ghi, thời gian thực thi, thông tin lỗi nếu có |
| UC07.4 | Kích hoạt Reconciliation Job | ADMIN | UC này cho phép ADMIN khởi chạy thủ công job đối soát — tìm giao dịch PENDING vượt timeout → đánh dấu PENDING_TIMEOUT → tổng hợp discrepancy |
| UC07.5 | Xem kết quả Reconciliation | ADMIN | UC này cho phép xem kết quả phiên đối soát gần nhất: trạng thái (MATCH / MISMATCH / FAILED), số giao dịch PENDING_TIMEOUT, discrepancy items và trạng thái resolve |
...
| UC08.12 | Xác nhận (Acknowledge) báo cáo | MANAGER | UC này cho phép MANAGER xác nhận đã đọc và chấp thuận nội dung báo cáo phân tích, chuyển trạng thái báo cáo từ PUBLISHED sang ACKNOWLEDGED |
...
#### Chi tiết: Xem giao dịch (ANALYST, MANAGER, ADMIN)

Người dùng tương tác qua các màn hình:
1. Danh sách giao dịch (mặc định, phân trang)
2. Tùy chọn: áp dụng bộ lọc
3. Tùy chọn: click một dòng để xem chi tiết
4. Tùy chọn: click "Xuất file" → chọn định dạng → tải file (UC05.3)

**Quan hệ use case con:**
- **Xem giao dịch** `<<extend>>` **Lọc danh sách giao dịch**

| Use Case con | Actor | Quan hệ | Mô tả |
|---|---|---|---|
| Lọc danh sách giao dịch | ANALYST, MANAGER, ADMIN | `<<extend>>` từ Xem giao dịch | UC này cho phép người dùng thu hẹp kết quả theo: khoảng thời gian, trạng thái, kênh giao dịch (ATM / POS / ONLINE), ngưỡng fraud_score |
...
#### Chi tiết: Xem hồ sơ vay

**Quan hệ use case con:**
- **Xem hồ sơ vay** `<<extend>>` **Lọc danh sách hồ sơ vay**

| Use Case con | Actor | Quan hệ | Mô tả |
|---|---|---|---|
| Lọc danh sách hồ sơ vay | OPERATOR, REVIEWER | `<<extend>>` từ Xem hồ sơ vay | UC này cho phép người dùng lọc hồ sơ vay theo trạng thái, mức rủi ro (LOW / MEDIUM / HIGH), khoảng thời gian nộp |
...
**Quan hệ use case con:**
- **Phê duyệt / Từ chối hồ sơ vay** `<<include>>` **Xem hồ sơ vay** (UC03.3)
...
**Quan hệ use case con:**
- **Nhận case (Assign)** `<<include>>` **Xem case** (UC04.1) — phải vào danh sách trước
- **Nhận case (Assign)** `<<include>>` **Xác nhận nhận case**
- **Xem case** `<<extend>>` **Lọc và sắp xếp danh sách case**

| Use Case con | Actor | Quan hệ | Mô tả |
|---|---|---|---|
| Lọc và sắp xếp danh sách case | REVIEWER | `<<extend>>` từ Xem case | UC này cho phép REVIEWER lọc case theo khoảng fraud_score, sắp xếp theo thời gian chờ (lâu nhất lên đầu) |
...
> **Ràng buộc phân quyền:** Case phải ở trạng thái ASSIGNED (không thể quyết định case OPEN). REVIEWER chỉ quyết định được case được gán cho mình; MANAGER quyết định được bất kỳ case ASSIGNED nào (bypass ownership check).

**Quan hệ use case con:**
- **Ra quyết định Phê duyệt / Từ chối** `<<include>>` **Xem case** (UC04.1) — phải đọc trước khi quyết định
...
#### Chi tiết: Xem Dashboard tổng quan (ANALYST, MANAGER)
...
| Use Case con | Actor | Quan hệ | Mô tả |
|---|---|---|---|
| Xem KPI Cards và biểu đồ xu hướng | ANALYST, MANAGER | `<<include>>` từ Xem Dashboard | UC này cho phép người dùng xem các chỉ số: tổng giao dịch, tỉ lệ APPROVED / REJECTED / MANUAL_REVIEW, số case tồn đọng, tỉ lệ gian lận bị chặn |
| Lọc Dashboard theo thời gian | ANALYST, MANAGER | `<<extend>>` từ Xem Dashboard | UC này cho phép người dùng chọn khoảng thời gian (hôm nay / tuần này / tháng này) để cập nhật toàn bộ số liệu Dashboard |
| Xem chi tiết một KPI cụ thể | ANALYST, MANAGER | `<<extend>>` từ Xem Dashboard | UC này cho phép người dùng click vào một chỉ số KPI để xem số liệu chi tiết theo từng ngày trong khoảng đã chọn |

---

#### Chi tiết: Xuất báo cáo (CSV / PDF) (MANAGER, ADMIN)

Người dùng đang ở màn hình "Xem giao dịch" (UC02.2) → click "Xuất file" → chọn định dạng → tải file.

**Quan hệ use case con:**
- **Xuất báo cáo (CSV / PDF)** `<<extend>>` **Xem giao dịch** (UC02.2) — kích hoạt có điều kiện từ màn hình UC02.2
...
- **Xem Audit Log hệ thống** `<<extend>>` **Truy vết lịch sử một giao dịch cụ thể** (UC05.5)
...
*Lưu ý: "Truy vết lịch sử một giao dịch cụ thể" (UC05.5) là use case độc lập trong sơ đồ tổng quan. Tại đây nó là mở rộng có điều kiện từ "Xem Audit Log" — được kích hoạt khi người dùng chọn tra cứu một giao dịch cụ thể.*
...
Người dùng tương tác qua các màn hình:
1. Trang ETL — xem danh sách Data Lake Snapshots để kiểm tra dữ liệu có sẵn
...
- **Xem ETL Logs** `<<extend>>` **Xem chi tiết một ETL Job** — chỉ khi click vào dòng log
...
- **Xem danh sách Reconciliation Jobs** `<<extend>>` **Xem chi tiết Reconciliation Job**  — chỉ khi click vào một job
...
**Ràng buộc nghiệp vụ:** Chỉ ANALYST được cập nhật ngưỡng. Mọi thay đổi ghi Audit Log.
...
3. MANAGER xem danh sách báo cáo PUBLISHED (UC08.10)
4. MANAGER xem chi tiết và tải PDF (UC08.11)
5. MANAGER xác nhận đã đọc (UC08.12) → trạng thái ACKNOWLEDGED
...
| Nhập ghi chú xác nhận | MANAGER | `<<include>>` từ Acknowledge báo cáo | UC này cho phép MANAGER nhập ghi chú xác nhận (tùy chọn) trước khi đổi trạng thái báo cáo sang ACKNOWLEDGED |
